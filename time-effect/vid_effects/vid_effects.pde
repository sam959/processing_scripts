  ...
  if(isVideoPlaying && frameCount > 0){
    ...
    for (int y = 0; y < video.height; y++) {  
      ...
      for (int x = 0; x < video.width; x++) {
        ...
        if(effectOn){
            if(brightness(originalColor) < 100 ){
              ...
              int ind = i + 1 > video.pixels.length -1? 0 : i + 1;
        ...
        if(timeMachineActive){       
          ...
          buffCol = frameFromBuffer.pixels[mult];
      ...
      } else if(!timeMachineActive && !effectOn) {
        int in = i;
        PImage im = (PImage) buffers.get(int(random(buffers.size())));
        if(toggleDivide){
           float p = i - sin(millis() * 0.1);
...
void FX(){
    ...
    print(String.format("Switched to effect on: %b\n", effectOn));
}
